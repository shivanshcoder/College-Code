...
void EliminateEquation(PEquation Equation, PEquation Equation2, PEquation *SimplifiedEqation, int NumberTerms) {
	float Coefficient = Equation2->Coefficients[0];

	(*SimplifiedEqation)->Ans = Equation2->Ans - (Equation->Ans / Equation->Coefficients[0]) * Coefficient;

	for (int i = 0; i < NumberTerms; ++i) {
		(*SimplifiedEqation)->Coefficients[i] = Equation2->Coefficients[i] - (Equation->Coefficients[i] / Equation->Coefficients[0])*Coefficient;
	}
